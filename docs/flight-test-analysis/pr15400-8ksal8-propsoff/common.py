"""Shared loading for the @8ksal8 props-off corpus.
...
# (group label, controller, Airmode feature, log stems) - four arms per cell.
GROUPS = [
    ('ADRC, Airmode feature on',  'ADRC',    'on',  'b9_Airmode_on_ADRC',
     ['btfl_001', 'btfl_002', 'btfl_003', 'btfl_004']),
    ('ADRC, Airmode feature off', 'ADRC',    'off', 'b9_Airmode_switch_ADRC',
     ['btfl_005', 'btfl_006', 'btfl_007', 'btfl_008']),
    ('CLASSIC, Airmode feature on',  'CLASSIC', 'on',  'b9_Airmode_on_PID',
     ['btfl_021', 'btfl_022', 'btfl_023', 'btfl_024']),
    ('CLASSIC, Airmode feature off', 'CLASSIC', 'off', 'b9_Airmode_switch_PID',
...
def headers(stem, basedir=None):
    ...
    with gzip.open(os.path.join(basedir or HERE, f'{stem}.bbl.gz'), 'rb') as fh:
        blob = fh.read()
    # The header is a contiguous block of "H ..." lines at the start of the
    # file; scanning the whole binary for that pattern can fabricate keys out
    # of frame data (it did, once, on one log in this corpus).
...
def rotor_hz_per_motor(d, stem):
    """Median mechanical rotor frequency of EACH motor, Hz, from the logged eRPM.

    The field is eRPM/100 (blackbox.c:292) and eRPM is electrical, so the
    mechanical rate needs the pole pair count from the header. Per-motor is the
    only honest form: pooling four motors into one median hid a near-coincidence
    between one motor's rate and the yaw spectral line in this very corpus.
    """
...
def rotor_hz(d, stem):
    """Pooled all-motor median - kept ONLY for the group-level idle comparison,
    where the question is "how fast are the motors turning overall". Never use
    this to test whether a spectral line is a rotor order; use
    rotor_hz_per_motor() and check aliases against the real sample rate."""
    return float(np.median(rotor_hz_per_motor(d, stem)))
