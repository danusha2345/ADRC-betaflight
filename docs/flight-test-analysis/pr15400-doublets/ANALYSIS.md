...
1. **ADRC-021, now on two crafts**: the shipped b0 throttle law
   ...
   exponent. Both estimators (direct regression and the observer's own z3)
   agree the ESO over-estimates its control authority everywhere except
   right at hover.
...
*.bbl`, blackbox-tools `f832acf9cd`, no flags), then every number in this
document regenerates from the committed scripts — `identify_b0.py` /
...
`analyze_jmsweng.py` (second-craft overview, z3 rail episodes, wide-band
scan), `cascade_scan.py` (p3 experiment). The intermediate `.csv`/`.event`
decode products are not committed — they are byte-reproducible from the
`.bbl` with the pinned decoder.
...
collective exponent of **0.88** (an independent review implementation got
~0.74) — far from the 2 the shipped law implies below its cap; note this is
